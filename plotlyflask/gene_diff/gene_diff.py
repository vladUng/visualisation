...
def create_dropdown(files, plot_type, text="Select a file"):
    ...
    return html.Div(
        children=[
            html.H6(text),
            dcc.Dropdown(id="select-file-{}".format(plot_type), value=files[0], options=[{"label": i, "value": i} for i in files]),
        ]
    )
...
def create_config_vulcano(files):
    ...
    return html.Div(
        ...
        children=[
            ...
            html.Br(),
            dcc.RangeSlider(id="default-volcanoplot-input", min=-3, max=3, step=0.05, marks={i: {"label": str(i)} for i in range(-3, 3)}, value=[-1.0, 1]),
            html.Br(),
...
def init_callbacks(dash_app):
    ...
    @dash_app.callback(
        [Output("volcano-text-output", "children"), Output("click-data", "children"), Output("figure-volcano", "figure"), Output("figure-scatter", "figure")],
        [Input("plot-volcano", "n_clicks"), Input("default-volcanoplot-input", "value"), Input("figure-volcano", "selectedData")],
    ...
    @dash_app.callback(
        [Output("pi-plot-text-output", "children"), Output("figure-pi-plot", "figure"), Output("click-data-pi", "children")],
...
layout = html.Div(
    children=[
        ...
        html.Div(
            id="parrent-all-volcano",
            children=[
                html.Hr(),
                html.Div(id="figure-contols-volcano", style={"column-count": "1"}, children=[create_config_vulcano(files), create_config_piplot(files)]),
                html.Hr(),
                html.Br(),
            ],
        ),
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        dcc.Graph(id="figure-volcano", figure={"data": {}, "layout": {"title": "No Volcano displayed", "height": 900}}),
                    ]
                ),
                html.Div(
                    children=[
                        html.Div(id="pi-plot-text-output"),
                        dcc.Graph(id="figure-pi-plot", figure={"data": {}, "layout": {"title": "No π plot displayed", "height": 900}}),
                    ]
                ),
            ],
        ),
        html.Div(
            id="scatter-plot",
            children=[
                dcc.Graph(id="figure-scatter", figure={"data": {}, "layout": {"title": "No Scatter plot displayed", "height": 600}}),
                dcc.Markdown(
                    """
